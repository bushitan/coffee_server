...
class WmCreate:
    def __init__(self):
        ...
        # self.base_path = r"E:\CarcerWorld\方案策划\6 咖啡地图 2019-2-11\1 集点卡\外卖卡券\制作\\"
        self.base_path = r"E:\wm\\"
    ...
    '''
        @method 获取二维码数组
    '''
    def get_ticket_web(self,store_id,start ,end):
        '''
            测试web请求
        '''
        url = "http://www.51zfgx.com/dev/lite/print/get/wm_list/"
        data = {
            'token': 'bushitan',
            'start': start,
            'end': end,
        ...
        print("wm_list",wm_list )
        for i in wm_list:
            if  i['store_id'] != store_id:
                print ( u"店铺store_id不对,外卖编号：%s，店铺：%s" % ( i['short_id'], i['store_id'] ))
                raise
    ...
    '''
        @method 2 获取二维码
    '''
    def getQrImage(self,access_token ,data,file_path):
    ...
    def add_qr_in_file(self,file_path,ticket_list):
        ...
        token = self.getToken("wxd19bbe9cb3b293b6", "931ad8364ea6a1327ed65282af330415")

        for t in ticket_list:
            short_uuid = t['short_uuid']
            # qr_path = file_path + "wm_%s_%s.png" %(t.id ,short_uuid )
            qr_path = file_path + "%s.png" %(t['short_id'] )  #  文件命名
            data = {
            ...
            self.getQrImage(token,data,qr_path)
...
class WmUtils():
    ...
    def save_image(self,store_id,start,end):
        ...
        ticket_list = wc.get_ticket_web(store_id,start,end)
...
if __name__  == '__main__':
    ...
    # wm_utils.save_image(25,94401 ,95000)

    # wm_utils.save_image(17,92601,	93200)
    # wm_utils.save_image(17,93201,	93800)
    # wm_utils.save_image(17,93801,	94400)
    ...
    # ticket_list = wc.get_ticket_web(1,1,2)
